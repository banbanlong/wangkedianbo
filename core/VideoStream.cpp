                                        ...
                                        unsigned *lines)
{
    ...
    case Vlc::RGBFormat:
        qstrcpy(chroma, "RV32");
        _frames.emplace(_frames.end(), new VlcRGBVideoFrame(width, height, pitches, lines));
        return 3;
    }
...
std::shared_ptr<VlcAbstractVideoFrame> VlcVideoStream::cloneFrame(std::shared_ptr<VlcAbstractVideoFrame> frame)
{
    switch (_format) {
        ...
        case Vlc::RGBFormat:
        {
            std::shared_ptr<VlcRGBVideoFrame> from = std::dynamic_pointer_cast<VlcRGBVideoFrame>(frame);
            if (from) {
                return std::make_shared<VlcRGBVideoFrame>(from);
            }
        }
    }
